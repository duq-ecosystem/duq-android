# DUQ Android — редизайн: чат → командный пульт OpenClaw

**Дата:** 2026-06-17
**Статус:** дизайн на ревью
**Решения Danny:** скелет Чат+Пульт+Лента+command palette; утка и тёмный неон
сохраняются; glassmorphism точечно; **только реальный API OpenClaw, никаких
...
движком OpenClaw**, не теряя текущий голосовой UX (утка, push-to-talk, TTS,
...
- **Только реальный API.** Каждый экран маппится на конкретные RPC. Нет данных
  от API → нет экрана. (Поэтому «умный дом» исключён — в OpenClaw нет нативного
...
- **RPC-слой:** в `OpenClawGatewayClient` добавить типизированные вызовы
...
- **Умный дом** — нет нативного API OpenClaw. Вернётся только при появлении
