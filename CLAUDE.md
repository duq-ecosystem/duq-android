...
#### ⛔ Восстановление node-пайринга (выстрадано часами 2026-06-19 — делать ИМЕННО так)
...
**Корневые грабли (все три обязательны):**
1. **При переустановке APK телефон теряет Ed25519-ключ → новый deviceId.** Operator
   перепейривается и живёт, а node-роль остаётся без токена под новый ключ. (Поэтому в
   `devices/paired.json` накапливаются несколько android-identity — это они.)
2. **setup-code живёт 10 минут** (`DEVICE_BOOTSTRAP_TOKEN_TTL_MS=600000`). Генерить
   `openclaw qr --setup-code-only` и вводить СРАЗУ, не отвлекаясь, иначе protunет → invalid.
3. **node-регистрацию с commands в `nodes/paired.json` создаёт ТОЛЬКО `openclaw nodes
   approve <reqId>`.** `openclaw devices approve` выдаёт node-ТОКЕН, но БЕЗ commands →
   `node did not declare any supported commands` на любой invoke. Это разные вещи!

**Процедура (чистый онбординг, надёжнее всего):**
```bash
# 1. чистое состояние телефона (новый ключ) + подними app (см. краш ниже)
adb shell pm clear com.duq.android
adb shell pm grant com.duq.android android.permission.ACCESS_FINE_LOCATION   # иначе FGS-краш
adb shell pm grant com.duq.android android.permission.POST_NOTIFICATIONS
adb shell am start -n com.duq.android/.MainActivity
# 2. На PairingScreen: "Enter code manually" → вставить свежий setup-code → Pair.
#    Код брать ТУТ ЖЕ (живёт 10 мин):  openclaw qr --setup-code-only --no-ascii | tail -1
# 3. approve operator-запроса (device-level):
openclaw devices approve <reqId-operator>      # reqId из devices/pending.json (role=operator)
# 4. рестарт app — node-клиент с node-токеном создаёт NODE-level pending:
adb shell am force-stop com.duq.android && adb shell am start -n com.duq.android/.MainActivity
openclaw nodes pending                          # появится node-запрос
openclaw nodes approve <reqId-node>             # ← создаёт регистрацию с commands
# 5. E2E (реальный результат, не логи):
openclaw nodes invoke --node <id> --command notify.show --params '{"title":"t","body":"b"}'
openclaw nodes invoke --node <id> --command location.get --params '{}'
# 6. убрать осиротевшие старые identity: openclaw nodes remove --node <старый-id>
```
