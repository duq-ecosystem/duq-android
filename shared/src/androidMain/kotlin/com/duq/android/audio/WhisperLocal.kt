...
/**
 * Android-реализация [LocalStt] — on-device STT через whisper.cpp (JNI):
 *  - модель ggml докачивается в filesDir при первом запуске;
...
class WhisperLocal(
    ...
    companion object {
        ...
        private val nativeAvailable: Boolean = try {
            ...
            Log.w(TAG, "libduqwhisper недоступна — on-device STT отключён, fallback на сервер: ${e.message}")
            false
        }
    }
    ...
    private val httpClient by lazy {
        ...
        // пока докачка модели идёт через системный DNS (github разрешается штатно).
    ...
    override fun isModelReady(): Boolean = modelFile().exists() && modelFile().length() > 1_000_000L
     ...
     * Возвращает текст ИЛИ null — если STT_ON_DEVICE выключен, нативная либа не собрана,
     * модель не готова (и не докачалась), либо распознавание упало/пусто. На null вызывающий
     * уходит на серверный /stt.
     */
    override suspend fun tryTranscribe(wavPath: String): String? {
        ...
        return try {
            ...
            transcribeWav(File(wavPath)).takeIf { it.isNotBlank() }
        } catch (e: Exception) {
