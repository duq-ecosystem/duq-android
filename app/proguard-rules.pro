...
# Anonymous `object : TypeToken<List<X>>() {}` subclasses: R8 strips their generic
# signature, so Gson throws "TypeToken must be created with a type argument" and
# every fromJson<List<…>> silently returns empty. This was the real cause of the
# empty notification inbox (Item fields were fine; the TypeToken read failed).
...
# BouncyCastle Ed25519 device identity (lightweight API; keep to be safe under R8)
