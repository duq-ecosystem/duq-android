...
class MessageTest {
    ...
    @Test
    fun `MessageRole fromApiString returns ASSISTANT for unknown role`() {
        // Unknown roles fall back to ASSISTANT (safe default, gateway may send new roles)
        assertEquals(MessageRole.ASSISTANT, MessageRole.fromApiString("unknown"))
        assertEquals(MessageRole.ASSISTANT, MessageRole.fromApiString(""))
        assertEquals(MessageRole.ASSISTANT, MessageRole.fromApiString("system"))
    }
