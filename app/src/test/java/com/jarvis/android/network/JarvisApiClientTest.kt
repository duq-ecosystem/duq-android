...
class JarvisApiClientTest {
    ...
    fun `ApiResult Success holds audio data and text`() {
        ...
        val text = "Hello"
        val result = JarvisApiClient.ApiResult.Success(audioData, text)
    ...
    fun `ApiResult Success with default empty text`() {
        ...
        val result = JarvisApiClient.ApiResult.Success(audioData)
    ...
    fun `ApiResult Success equality works correctly`() {
        val result1 = JarvisApiClient.ApiResult.Success(byteArrayOf(1, 2, 3), "test")
        val result2 = JarvisApiClient.ApiResult.Success(byteArrayOf(1, 2, 3), "test")
        val result3 = JarvisApiClient.ApiResult.Success(byteArrayOf(1, 2, 4), "test")
    ...
    fun `ApiResult Error holds message and code`() {
        val result = JarvisApiClient.ApiResult.Error("Not found", 404)
    ...
    fun `ApiResult Error code is optional`() {
        val result = JarvisApiClient.ApiResult.Error("Generic error")
    ...
    fun `TokenRefreshResult successful has correct fields`() {
        val result = JarvisApiClient.TokenRefreshResult(
            success = true,
    ...
    fun `TokenRefreshResult default values`() {
        val result = JarvisApiClient.TokenRefreshResult(success = true)
    ...
    fun `BASE_URL is not empty`() {
        assertTrue(JarvisApiClient.BASE_URL.isNotEmpty())
    }

    @Test
    fun `BASE_URL starts with https`() {
        assertTrue(JarvisApiClient.BASE_URL.startsWith("https://"))
    }
    ...
    private fun isRetryable(e: Exception): Boolean {
        val method = JarvisApiClient::class.java.getDeclaredMethod("isRetryableException", Exception::class.java)
