...
/**
 * On-device TTS через sherpa-onnx (k2-fsa) + Piper VITS RU. Заменяет серверный /tts (Silero):
 *  - бандл модели (vits-piper-ru_RU-irina-medium.tar.bz2) докачивается в filesDir при первом
 *    голосовом ответе и распаковывается (model.onnx + tokens.txt + espeak-ng-data);
 *  - синтез → WAV-файл (тот же контейнер, что отдавал сервер), играет [ChatAudioPlaybackManager].
 *
 * Зеркалит архитектуру [WhisperLocal] (on-device STT): единая точка [trySynthesize] инкапсулирует
 * флаг + докачку + синтез + fallback; `null` → вызывающий уходит на серверный /tts. Движок грузится
 * один раз и переиспользуется (init дорогой); выгружается при нехватке RAM (onTrimMemory).
 */
@Singleton
class TtsLocal @Inject constructor(
    ...
    /**
     * Единая точка on-device синтеза для всех вызывающих. Возвращает WAV-файл ИЛИ null —
     ...
     */
    suspend fun trySynthesize(text: String, messageId: String): File? {
        if (!AppConfig.TTS_ON_DEVICE || text.isBlank()) return null
        return try {
            if (!isModelReady()) ensureModel()
            if (!isModelReady()) return null
            synthesizeWav(text, messageId)
        } catch (e: Exception) {
    ...
    private suspend fun synthesizeWav(text: String, messageId: String): File = withContext(Dispatchers.Default) {
        ...
        // что отдавал серверный Silero, ExoPlayer играет его без изменений.
