...
class WakeWordManager(
    ...
    fun start() {
        ...
        try {
            ...
            onError("Invalid Porcupine API key: ${e.message} ${e.messageStack?.joinToString(" | ") ?: ""}")
        } catch (e: PorcupineException) {
            // Porcupine hides the real cause in messageStack, not message.
            val stack = e.messageStack?.joinToString(" | ").orEmpty()
            Log.e(TAG, "Porcupine error: $stack", e)
            onError("Porcupine error: ${e.message} [$stack]")
        } catch (e: Exception) {
