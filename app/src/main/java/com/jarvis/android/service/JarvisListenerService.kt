...
class JarvisListenerService : Service() {
    ...
    val error: StateFlow<JarvisError?> = _error
    ...
    private val stateCallback = object : VoiceCommandProcessor.StateCallback {
        ...
        override fun onError(error: JarvisError) {
            _error.value = error
        }
    }
    ...
    private suspend fun startWakeWordManager() {
        try {
            ...
            if (apiKey.isBlank()) {
                ...
                _error.value = JarvisError.ConfigurationError.missingApiKey("Porcupine API key")
                Log.d(TAG, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                return
            }
            ...
            wakeWordManager = WakeWordManager(
                ...
                onError = { errorMessage ->
                    ...
                    _error.value = if (errorMessage.contains("API key", ignoreCase = true)) {
                        JarvisError.WakeWordError.invalidApiKey()
                    } else {
                        JarvisError.WakeWordError.initFailed(RuntimeException(errorMessage))
                    }
                }
            )
            ...
            e.printStackTrace()
            _state.value = JarvisState.ERROR
            _error.value = JarvisError.WakeWordError.initFailed(e)
            Log.d(TAG, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        }
    }
