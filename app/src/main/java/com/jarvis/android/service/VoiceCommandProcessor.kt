...
class VoiceCommandProcessor @Inject constructor(
    ...
    sealed class ProcessingResult {
        ...
        data class Error(val error: JarvisError) : ProcessingResult()
        object RecordingFailed : ProcessingResult()
    }

    interface StateCallback {
        fun onStateChanged(state: JarvisState)
        fun onError(error: JarvisError)
    }
        ...
        return try {
            ...
            when (result) {
                ...
                is JarvisApiClient.ApiResult.Error -> {
                    ...
                    Log.e(TAG, "═══════════════════════════════════════")
                    val error = when {
                        result.code == 401 || result.code == 403 -> JarvisError.AuthError.invalidToken()
                        result.code != null && result.code >= 500 -> JarvisError.NetworkError.serverError(result.code, result.message)
                        result.code != null -> JarvisError.NetworkError.clientError(result.code, result.message)
                        result.message.contains("timeout", ignoreCase = true) -> JarvisError.NetworkError.timeout(result.message)
                        result.message.contains("connection", ignoreCase = true) -> JarvisError.NetworkError.noConnection(result.message)
                        else -> JarvisError.NetworkError(result.message, result.code)
                    }
            ...
            Log.e(TAG, "═══════════════════════════════════════")
            val error = when (e) {
                is java.net.SocketTimeoutException -> JarvisError.NetworkError.timeout()
                is java.net.UnknownHostException -> JarvisError.NetworkError.noConnection()
                is java.net.ConnectException -> JarvisError.NetworkError.noConnection("Connection refused: ${e.message}")
                is java.io.IOException -> JarvisError.NetworkError(e.message ?: "Network error")
                else -> JarvisError.AudioError(e.message ?: "Unknown error", e)
            }
            callback.onError(error)
            ProcessingResult.Error(error)
        } finally {
